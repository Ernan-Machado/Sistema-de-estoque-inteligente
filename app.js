...
async function cadastrarProduto() {
    ...
    const { error } = await _supabase.from('products').insert([{ nome, sku, preco, estoque_minimo, estoque_atual: 0 }]);
...
async function carregarEstoque() {
    const { data: produtos, error } = await _supabase.from('products').select('*').order('nome', { ascending: true });
...
function filtrarProdutos() {
    ...
    const linhas = document.getElementById('lista-produtos').getElementsByTagName('tr');
...
async function alterarQtd(id, atual, tipo) {
    const qtd = parseInt(document.getElementById(`mov-${id}`).value);
